...
def plot_accuracy():
    ...
    def plot_one(df, label, xpoints):
        ...
        ax1.plot(xpoints, accuracy_means, label=label, color=color[label], linestyle=linestyle[label])

    if baseline:
        plot_one(df_baseline, label="baseline", xpoints = num_examples)
    
    if union_all:
        plot_one(df_union_all, label="BC-union-all", xpoints = num_examples)
   
    if union_few:
        plot_one(df_union_few, label="BC-union-few", xpoints = num_examples)
    
    if intersect:
        plot_one(df_intersect, label="BC-intersect", xpoints = num_examples)

    plt.grid(axis='y')

    ax1.set_xticks(num_examples)
...
def plot_number_programs():
    ...
    ax1.get_yaxis().set_major_formatter(matplotlib.ticker.ScalarFormatter())
    plt.xlabel("# positive examples")
    plt.ylabel("# searched programs")
    plt.legend()
    plt.title("Number of searched programs")
...
def plot_total_learning_time():
    ...
    if baseline:
        plot_one(df_baseline, label="baseline", column="learning_time", xpoints = num_examples)
    
    if union_all:
        plot_one(df_union_all, label="BC-union-all", column="total_learning_time", xpoints = num_examples)
    
    if union_few:
        plot_one(df_union_few, label="BC-union-few", column="total_learning_time", xpoints = num_examples)    

    if intersect:
        plot_one(df_intersect, label="BC-intersect", column="total_learning_time", xpoints = num_examples)
    ...
    plt.xlabel("# positive examples", fontsize=14)
...
def plot_learning_time_excluding_pruning():
    ...
    if baseline:
        plot_one(df_baseline, label="baseline", column="learning_time", xpoints = [x for x in num_examples])
    
    if union_all:
        plot_one(df_union_all, label="BC-union-all", column="learning_time", xpoints = num_examples)
    
    if union_few:
        plot_one(df_union_few, label="BC-union-few", column="learning_time", xpoints = [x for x in num_examples])
    
    if intersect:
        plot_one(df_intersect, label="BC-intersect", column="learning_time", xpoints = [x for x in num_examples])
...
def plot_bottom_clause_time():
    ...
    def plot_one(df, label, xpoints):
        ...
        ax1.errorbar(num_examples, bottom_clause_construction_time_means, yerr=bottom_clause_construction_time_std, label=label, color=color[label], linestyle=linestyle[label])
...
def plot_removed_preds():
    ...
    if union_few:
        plot_one(df_union_few, label = "BC-union-few", xpoints = [x + 1 for x in num_examples])
    
    if intersect:
        plot_one(df_intersect, label = "BC-intersect", xpoints = [x + 1 for x in num_examples])
...
def plot_removed_preds_percentage():
    ...
    def plot_one(df, label, xpoints):
        removed_preds = [df[i]["removed_preds"] / 500 * 100 for i in range(num_tasks)] 
        removed_preds_means = [data.mean() for data in removed_preds]
        removed_preds_std = [data.std() for data in removed_preds]
        ax1.errorbar(xpoints, removed_preds_means, yerr=removed_preds_std, label = label, color=color[label], linestyle=linestyle[label])
