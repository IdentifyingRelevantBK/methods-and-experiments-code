...
EXAMPLES_BY_TASK = range(20, 201, 20)

NUM_SHAPES = 500
SHAPES = [f"shape{i}" for i in range(NUM_SHAPES)]


def generate_zendo_problem(num_train_pos, output_dir):
...
def generate_all_tasks():
    for test_id in range(len(EXAMPLES_BY_TASK)):
        print(f"generating task {test_id}")
        generate_zendo_problem(EXAMPLES_BY_TASK[test_id], f"task{test_id}")
